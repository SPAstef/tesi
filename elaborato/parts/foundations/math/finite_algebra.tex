...
Elements of different algebraic structures can be associated through \emph{morphisms}.
...
Fields are one of the most important and studied algebraic structures: the algebraic structure of 
...
\begin{example}
  ...
  (see \Cref{ex:endo_group,ex:endo_monoid}), and it is easy to show that \(\circ \)
...
\begin{definition}[Dot product]
  ...
  \]
\end{definition}
\begin{definition}[Vector rotation]
...
\begin{definition}[Diagonal vector]
...
standard way.
\begin{definition}[Lagrange interpolation~\cite{Waring1779}]
...
By using interpolation, we can build the unique polynomial of degree \(n - 1\) which, 
